...
function store({ serial = false, executionSerial = false, missingLifecycle = false, alteredLineage = false } = {}) {
  const writers = [
    { id: "task-alpha", role: "backend", deliveryRunId: "run-1", executionIsWriter: true, requirementIds: ["alpha"], allowedPaths: ["src/alpha.mjs"], planBatchId: "batch-1", wave: 1, dependencies: [], executionDependencies: [] },
    { id: "task-beta", role: "backend", deliveryRunId: "run-1", executionIsWriter: true, requirementIds: ["beta"], allowedPaths: ["src/beta.mjs"], planBatchId: "batch-1", wave: 1, dependencies: serial ? ["task-alpha"] : [], executionDependencies: executionSerial ? ["task-alpha"] : [] }
  ];
  ...
  return {
    ...
    listTasks: () => writers,
    planBatch: (id) => id === "batch-1" ? { id, deliveryRunId: "run-1", blueprintId: "pb-1", wave: 1, tasks: [{ id: "write-alpha", primaryDomain: "backend", requirementIds: ["alpha"], allowedPaths: ["src/alpha.mjs"] }, { id: "write-beta", primaryDomain: "backend", requirementIds: ["beta"], allowedPaths: ["src/beta.mjs"] }] } : null,
    events: () => events,
...
test("controller verification fails closed for unknown capabilities, identities, serial turns, lifecycle gaps, and altered lineage", () => {
  ...
  assert.equal(evidence({ executionSerial: true }).status, "not_verified");
...
test("final acceptance admits controller evidence only with exact candidate and checkpoint binding", () => {
  ...
  report.results[1].evidence[1].controllerExecution.checkpointSha = candidateSha;
  report.results[1].evidence[1].controllerExecution.lifecycleIntervals = [];
  assert.throws(() => validateProductAcceptanceReport(report, { blueprint, blueprintDigest: "c".repeat(64), manifest: { id: "manifest-1", candidateSha }, manifestPath: "out/manifest.json" }), /controller evidence identity/);
});

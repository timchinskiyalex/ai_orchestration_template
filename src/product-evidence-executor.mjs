...
export class ProductEvidenceExecutor {
  ...
  async verify({ candidate, manifest: integration, deliveryRunId }) {
    try {
      ...
      for (const commandId of [...new Set(verification.mappings.map((mapping) => mapping.commandId))]) {
      ...
      const evidence = { candidateSha: candidate.sha, results: verification.mappings.map((mapping) => ({ requirementId: mapping.requirementId, criterionId: mapping.criterionId, testId: mapping.testId, reference: `${id}:${mapping.testId}`, candidateSha: candidate.sha, status: resultFor.get(mapping.commandId)?.result === "pass" ? "pass" : "failed" })) };
      ...
      this.router.store.recordProductEvidenceExecution({ id, ...identity, verificationManifest: persistedManifest, commands: commandResults, evidence, success: commandResults.every((item) => item.result === "pass") });
      return evidence;
    } catch {

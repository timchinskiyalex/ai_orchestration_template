...
export function sourceIntakeFailure({ role, phase, code, receipt = null, errorClass = null }) {
  ...
  const receiptIdentity = receipt && typeof receipt === "object" && ["threadId", "requestedTurnId", "resolvedTurnId"].every((key) => typeof receipt[key] === "string" && receipt[key])
    ...
    : null;
  const failure = Object.freeze({ schemaVersion: SOURCE_INTAKE_FAILURE_SCHEMA_VERSION, role: failureRole, phase, code, receiptIdentity, diagnostics: typeof errorClass === "string" && /^[a-z][a-z0-9_-]{0,63}$/i.test(errorClass) ? { errorClass } : null });

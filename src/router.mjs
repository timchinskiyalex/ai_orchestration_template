...
export class SwarmRouter extends EventEmitter {
  ...
  async publishCandidate(integration, { confirmRemotePush = false, remoteGitAdapter = null, pullRequestAdapter = null, remoteCiAdapter = null, mergeAdapter = null, acceptanceReportId = null } = {}) {
    ...
    // A disabled remote is an explicit local-delivery mode, not a credentials
    // failure. It may only complete after the exact candidate has a persisted
    // passing ProductAcceptanceReport; no remote adapter is constructed here.
    if (!remote.enabled) {
      const acceptance = acceptanceReportId ? this.store.productAcceptanceReport(acceptanceReportId) : null;
      if (!acceptance || !acceptance.passing || acceptance.report.deliveryRunId !== run.id || acceptance.report.integrationManifestId !== manifest.id || acceptance.report.candidateSha.toLowerCase() !== candidate.sha.toLowerCase()) return { terminalState: "awaiting_final_acceptance", status: "awaiting_final_acceptance", candidate, localCandidate: true, reason: "Local candidate completion requires a matching persisted passing ProductAcceptanceReport." };
      return { terminalState: "completed_candidate_ready", status: "completed_candidate_ready", candidate, acceptanceReportId: acceptance.id, localCandidate: true, remoteEnabled: false };
    }
    if (!auto && !confirmRemotePush) return { terminalState: autonomous ? "blocked_credentials" : "awaiting_human", status: autonomous ? "blocked_remote" : "awaiting_human_remote_handoff", reason: "Remote publication is disabled by autonomy policy.", candidate };
  ...
  async #runTask(client, task) {
    ...
    let overlayContext = (ENGINEERING_DOMAINS.has(task.role) || (task.role === "bootstrap" && existsSync(join(this.config.repository, this.config.project.documentationDir, "inventory.json")))) ? this.#workerOverlayContext() : null;
  ...
  async buildProductAcceptanceReport({ integration, remoteCi, productEvidence = null }) {
    ...
    const localOnly = this.config.remote?.enabled === false;
    const report = { schemaVersion: PRODUCT_ACCEPTANCE_SCHEMA_VERSION, kind: PRODUCT_ACCEPTANCE_KIND, deliveryRunId: run.id, blueprintId: stored.blueprint.blueprintId, blueprintDigest: stored.digest, documentSetDigest: stored.documentSetDigest, integrationManifestPath: integration.path, integrationManifestId: manifest.id, candidateSha: manifest.candidateSha, generatedAt: new Date().toISOString(), ...(repositoryBaseline ? { repositoryBaselineDigest: repositoryBaseline.digest, behaviorEvidence } : {}), evidence: { integration: { status: manifest.localVerification?.status === "passed" ? "pass" : "missing", reference: integration.path, candidateSha: manifest.candidateSha, kind: "integration-manifest" }, qa: { status: qaPass ? "pass" : "missing", reference: "quality_reports", candidateSha: manifest.candidateSha, kind: "qa-lineage" }, security: { status: securityPass ? "pass" : "missing", reference: "security_reports", candidateSha: manifest.candidateSha, kind: "security-lineage" }, productE2e: product, ci: { status: localOnly || (remoteCi?.status === "passed" && remoteCi?.candidateSha?.toLowerCase() === manifest.candidateSha.toLowerCase()) ? "pass" : "not_verified", reference: localOnly ? "remote-disabled-local-candidate" : remoteCi?.idempotencyKey ?? "remote-ci", candidateSha: manifest.candidateSha, kind: localOnly ? "local-candidate-no-remote" : "remote-ci" } }, results: [] };
  ...
  #assertStoredBlueprintSourceIntegrity(stored, { requireManifest = true } = {}) {
    const requiresRepositoryVerification = stored?.blueprint?.requirements?.some((requirement) => requirement.acceptanceCriteria?.some((criterion) => criterion.repositoryVerification));
    const projectOverlay = requiresRepositoryVerification ? this.#workerOverlayContext().overlay : null;
    if (!requireManifest) {
      try {
        return validateControllerAuthorizedBlueprint(stored.blueprint, { sourceResolver: this.#sourceEvidenceResolver(), policyRegistry: this.#controllerPolicyRegistry(), persistedResolutionAuthority: stored.resolutionAuthority, projectOverlay });
      } catch {
    ...
    try {
      return validateControllerAuthorizedBlueprint(stored.blueprint, { sourceResolver: this.#sourceEvidenceResolver(), policyRegistry: this.#controllerPolicyRegistry(), persistedResolutionAuthority: stored.resolutionAuthority, sourceClaimManifest, projectOverlay });
    } catch {
      throw new Error("source_claim_contract:persisted_blueprint_source_validation_failed");
    }
  }
